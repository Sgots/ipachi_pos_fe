...
const Sidebar: React.FC = () => {
  ...
  const hookLogo = useBusinessLogo(businessLogoUrl || undefined, Boolean(businessLogoUrl));
  ...
  const items: Item[] = useMemo(() => {
    if (!hasActive && !loading) {
      return [{ to: "/account", label: "Account", requireActive: false }];
    }

    if (isDashboardOnlyPlan) {
      const dashboardOnlyItems: Item[] = [
        { to: "/dashboard", label: "Dashboard", requireActive: true },
        { to: "/account", label: "Account", requireActive: false },
      ];

      return dashboardOnlyItems.filter((item) => {
        if (shouldHideDashboard && item.to === "/dashboard") return false;
        return true;
      });
    }

    const fullMenuItems: Item[] = [
      { to: "/dashboard", label: "Dashboard", requireActive: true },
  ...
  }, [hasActive, loading, isDashboardOnlyPlan, shouldHideDashboard]);
    ...
    >
      ...
      <div className="relative px-4 pt-4 pb-2">
        ...
        <div className="mt-2 flex justify-center">
          <div
            className={`${collapsed ? "h-16 w-16" : "h-24 w-24"} rounded-full shadow-md ring-1 flex items-center justify-center`}
            style={{
      ...
      <nav className="mt-4 px-3 space-y-1">
        ...
        {items.map((it) => (
          <NavItem
            ...
            icon={it.icon}
            collapsed={collapsed}
            disabled={Boolean(it.requireActive && !hasActive)}
          />
        ))}
      </nav>
