...
export const setAuthToken = (token: string | null) => {
    // Persist and prime axios default header
...
/** PRIME token on module load so first request after refresh includes Authorization */
(() => {
    try {
        const t = getLS("auth.token")
            || (() => {
                try { return JSON.parse(getLS("ipachi_user") || "{}")?.token; } catch { return null; }
            })();
...
client.interceptors.request.use((config) => {
    ...
    // Authorization (ensure present for non-auth)
    if (!isPublicAuth) {
        let token = localStorage.getItem("auth.token");
        if (!token) {
            try { token = JSON.parse(localStorage.getItem("ipachi_user") || "{}").token; } catch { /* noop */ }
        }
        if (token && token !== "null" && token !== "undefined") {
            if (typeof (config.headers as any).set === "function") {
