...
const fmtMoney = (n: number | string | (number | string)[] | null | undefined) =>
  ...
  }).format(Number(Array.isArray(n) ? n[0] : n ?? 0));
...
const monthRange = (dateStr: string) => {
  ...
  return {
    start: toIsoStart(ymd(start)),
    end: toIsoEnd(ymd(end)),
  };
};
...
const Reports: React.FC = () => {
  ...
  // Requirement:
  // DISCOVER = limited report.
  // Any other active plan = full comprehensive report.
  const isFullPlan = Boolean(hasActive) && !isDiscoverPlan;

  /* ---------- Business-ID handling ---------- */

const readBiz = () => localStorage.getItem("x.report.business.id") || "";
const readSelectedBusinessName = () => localStorage.getItem("x.report.business.name") || "";

const [bizId, setBizId] = useState<string>(readBiz());
const [selectedBusinessName, setSelectedBusinessName] = useState<string>(readSelectedBusinessName());
...
const applyHeaders = useCallback(() => {
  ...
  console.log("Reports using business ID:", bizId);
  console.log("Reports using business name:", reportBusinessName);
}, [bizId, reportBusinessName]);

useEffect(() => {
  applyHeaders();

  return () => {
    // Prevent selected SME report context from leaking into Account page.
    delete (client as any).defaults.headers.common["X-Business-Id"];
  };
}, [applyHeaders]);

useEffect(() => {
  setSelectedBusinessName(readSelectedBusinessName());
}, [bizId]);
 ...
 useEffect(() => {
  ...
  const loadAll = async () => {
    ...
    if (!hasActive) {
      setErrNote("Your plan is not active.");
      resetAllData();
      return;
    }
...
console.log("Loading reports for business ID:", bizId);
console.log("X-Business-Id header:", (client as any).defaults.headers.common["X-Business-Id"]);
...
setLoading(true);
    ...
    try {
      /*
        DISCOVER must only load and show:
        1. Sales by Product
        2. Sales Trends
        3. Best 3 Performers
        ...
        Full plans load the above plus dashboard KPIs, Cash-Up,
        Trade Account Statement, and derived comments.
      */
      ...
      const [
        ...
        client.get<ApiResponse<any[]>>("/api/reports/best-performers", {
          params: { ...params, top: 3 },
        }),
        ...
        client.get<ApiResponse<any[]>>("/api/reports/sales-by-product", {
          params: { start: cmStart, end: cmEnd },
        }),
        client.get<ApiResponse<any[]>>("/api/reports/sales-by-product", {
          params: { start: pmStart, end: pmEnd },
        }),
      ]);

      setSalesByProduct(
        arr(sbp?.data).map((r: any) => ({
          name: r.name,
...
// Dashboard KPIs are available to DISCOVER, MONITOR and INTELLIGENCE.
const { data: dash } = await client.get<ApiResponse<any>>("/api/reports/dashboard", {
  params,
});
...
setKpis(
  dash?.data ?? {
    ...
    topProduct: "-",
      ...
      // Any other active plan gets full reports.


      if (reportType === "Cash Up" || reportType === "Comprehensive") {
        const { data } = await client.get<ApiResponse<any>>("/api/reports/cashup", {
          params,
        });
      ...
      if (reportType === "Trade Account Statement" || reportType === "Comprehensive") {
        const { data } = await client.get<ApiResponse<any>>("/api/reports/trade-account", {
          params,
        });
      ...
      if (status === 403) setErrNote("Access denied. Check user permissions or headers.");
      else if (status === 400) setErrNote("Bad request. Verify selected dates.");
  ...
  }, [params.start, params.end, bizId, reportType, CAN_VIEW_REPORTS, hasActive, isDiscoverPlan]);
  ...
  const comments = useMemo(() => {
    ...
    const best = salesByProduct.reduce(
      (a, b) => (Number(b?.value) > Number(a?.value) ? b : a),
      salesByProduct[0] ?? null
    );

    const least = salesByProduct.reduce(
      (a, b) => (!a || Number(b?.value) < Number(a?.value) ? b : a),
      null as any
    );
    ...
    if (prevMonthByProduct.length || currMonthByProduct.length) {
      ...
      if (prevTotal > 0) {
        ...
        keys.forEach((k) => {
          ...
          deltas.push({
            name: curM.get(k)?.name ?? preM.get(k)?.name ?? "",
            delta: c - p,
          });
        });
    ...
    list.push(
      `Business made a Gross ${grossPLValue >= 0 ? "Profit" : "Loss"} of BWP ${fmtMoney(
        Math.abs(grossPLValue)
      )}.`
    );
    ...
    if (least) {
      list.push(`${least.name} is the least contributor to total sales and may require attention.`);
    }
    ...
    return list;
  }, [isFullPlan, kpis, salesByProduct, currMonthByProduct, prevMonthByProduct, ta]);

  /* ---------- Business ID save ---------- */

const handleSaveBiz = () => {
  const cleanBizId = bizId.trim();

  localStorage.setItem("x.report.business.id", cleanBizId);

  // Manual entry has no selected business name, so clear stale report name.
  ...
  const setPrintableRef = useCallback((node: HTMLDivElement | null) => {
    ...
    setPrintableReady(Boolean(node));
  }, []);

  const handlePrint = useReactToPrint({
    contentRef: printableRef,
documentTitle: `${reportBusinessName} Report For A Period Starting ${start} to ${end}`,
    onBeforeGetContent: () => new Promise((resolve) => setTimeout(resolve, 150)),
      ...
      @media print {
        * { box-sizing: border-box; }
        ...
        [data-export="section"][data-title="Trade Account Statement"] .MuiGrid-item {
          ...
          flex: 0 0 calc(16.6667% - 6.4px) !important;
        }
      }
    `,
  ...
  const renderValueLabel = (props: any) => {
    ...
    if (Number(value) < 1000) return null;
  ...
  if (!hasActive) {
    return (
      <Box sx={{ p: 3 }}>
        <Typography variant="h6" color="text.secondary">
          Your plan is not active. Please activate a plan to view Reports.
        </Typography>
      </Box>
    );
  }

 const showKpis = Boolean(hasActive);

 const showCash =
   isFullPlan &&
   (reportType === "Cash Up" || reportType === "Comprehensive");

 const showTA =
   isFullPlan &&
   (reportType === "Trade Account Statement" || reportType === "Comprehensive");

 const showGeneralComments = isFullPlan;

  return (
    <Box sx={{ p: 3, bgcolor: "#f5f5f5" }}>
     <Typography variant="h5" sx={{ mb: 0.5, fontWeight: 700, color: brand.dark }}>
       {reportBusinessName} Report
       {isDiscoverPlan && (
         <Chip
           label="Discover Plan - Limited"
           color="primary"
           size="small"
     ...
     <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
      ...
      {isDiscoverPlan && (
        <Alert severity="info" sx={{ mb: 3 }}>
         Discover Plan: You can view Dashboard KPIs, Sales by Product, Sales Trends,
         Best 3 Performers, Profit by Category, and Sales by Location.
        </Alert>
      )}
  ...
  >
    ...
    <Typography variant="body2" color="text.secondary">
      Reporting period: {start} to {end}
                ...
                {[
                  {
                    ...
                    value: Number(kpis.customersServed ?? 0).toLocaleString(),
                  },
                  {
                    label: "Total Sales",
                    value: `BWP ${fmtMoney(kpis.totalSales)}`,
                  },
                  {
                    label: "Total VAT",
                    value: `BWP ${fmtMoney(kpis.totalVat)}`,
                  },
                  {
                    label: "Overall Profit",
                    value: `BWP ${fmtMoney(kpis.overallProfit)}`,
                  },
                  {
        ...
        {/* Discover + Full plans: Sales by Product and Sales Trends */}
        ...
        {/* Discover + Full plans: Best 3, Profit by Category, Sales by Location */}
        <Grid container spacing={2} sx={{ mb: 3 }} className="print-row">
          <Grid item xs={12} md={4} className="print-inline w-33">
            <Paper
              sx={{
                ...
                <PieChart margin={{ top: 10, right: 10, left: 10, bottom: 36 }}>
                  ...
                  <Legend
                    ...
                    formatter={(value: any, entry: any) => {
                      ...
                      return <span style={{ fontSize: 11 }}>{`${value} — BWP ${fmtMoney(v)}`}</span>;
                    }}
                  />
                </PieChart>
              </ResponsiveContainer>
        ...
        {/* Full plans only: Cash-Up Report */}
        {showCash && (
          <div data-export="section" data-title="Cash-Up Report">
            ...
            >
              ...
              <Table size="small">
                ...
                <TableBody>
                  {cashRows.map((r, i) => (
                    <TableRow key={`${r.sku || "sku"}-${r.name || "name"}-${i}`}>
                      <TableCell>
        ...
        {/* Full plans only: Trade Account Statement */}
        {showTA && (
        ...
        {/* Full plans only: General Comments */}
        {showGeneralComments && (
          <div data-export="section" data-title="General Comments">
            ...
            >
              ...
              {comments.length > 0 ? (
                ...
                <Typography variant="body2" sx={{ color: brand.dark }}>
                  Review the data above for key insights.
                </Typography>
              )}
            </Paper>
          </div>
