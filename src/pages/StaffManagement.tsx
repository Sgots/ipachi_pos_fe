...
const StaffManagement: React.FC = () => {
  ...
  /** Extract server message from JSON, string, or Blob */
 const getServerMessage = async (err: any): Promise<string> => {
   console.log("[getServerMessage] err.response:", err.response);
   console.log("[getServerMessage] err.response.data:", err.response?.data);
   ...
   if (!data) {
     console.log("[getServerMessage] no data, falling back to error message:", err.message);
     return err.message || "";
   }

   if (typeof data === "string") {
     console.log("[getServerMessage] data is string:", data);
     return data.trim();
   }

   if (typeof data === "object" && !(data instanceof Blob)) {
     console.log("[getServerMessage] data is object:", data);
     return (
       data.message ??
       data.error ??
       data.detail ??
   ...
   if (data instanceof Blob) {
     try {
       ...
       console.log("[getServerMessage] blob text:", text);
       return text.trim();
     } catch (e) {
       console.log("[getServerMessage] blob parse error:", e);
       return "";
     }
   }

   console.log("[getServerMessage] no message found, falling back to error message:", err.message);
   return err.message || "";
 };
  /** Build a friendly message without surfacing "Request failed with status code 403" */
const buildFriendlyError = async (err: any): Promise<{ message: string; severity: ToastSeverity }> => {
  const status: number | undefined = err?.response?.status;
  const serverMsg = (await getServerMessage(err)).trim();
  const lower = serverMsg.toLowerCase();
  console.log("[buildFriendlyError] status:", status, "serverMsg:", serverMsg, "error:", err); // Debug log

  // ... rest of the function ...


    // Plan limit reached (STAFF seats)
    if ((status === 403 || status === 401 || status === 409) && lower.includes("staff user limit")) {
      const m = serverMsg.match(/\((\d+)\)/);
      const allowed = m?.[1];
      return {
        message:
          `You’ve reached your plan’s staff seat limit` +
          (allowed ? ` (${allowed}).` : `.`) +
          ` Remove a staff member or upgrade your plan to add more seats.`,
        severity: "warning",
      };
    }

    // No active subscription
    if (status === 402 || lower.includes("no active subscription")) {
      return {
        message: "Your subscription isn’t active. Activate or renew your plan to add staff.",
        severity: "warning",
      };
    }
    ...
    if (status === 409 || lower.includes("already exists")) {
      return {
        message: "That Staff Number is already in use. Please enter a different Staff Number.",
        severity: "error",
      };
    }
    ...
    if (status === 400) {
      return {
        message: serverMsg || "Please check the form details and try again.",
        severity: "error",
      };
    }

    // Generic fallback — never include axios' err.message
    return {
      message: serverMsg || "We couldn’t complete that action. Please try again.",
      severity: "error",
    };
  };
  ...
  const saveStaff = async () => {
    ...
    const payload = {
      ...
      terminalId: staffDraft.terminalId,
    };
    ...
    try {
      if (editingStaffId == null) {
        ...
        setToastMsg("Staff details updated");
      ...
      const fm = await buildFriendlyError(err);
      setToastMsg(fm.message);
      setToastSeverity(fm.severity);
      setToastOpen(true);
      // keep dialog open so the user can adjust or cancel
    }
  };
  ...
  const confirmResetPassword = async () => {
    if (!isAdmin || !staffToReset) return;
    try {
      const res = await api.post(`/api/staff/${staffToReset.id}/reset-password`, {});
      console.log("[Reset Password] payload:", res.data);
      const pwd = res?.data?.password ?? res?.data?.data?.password;
      if (pwd) {
        setPwValue(pwd);
        setPwWho(`${staffToReset.firstname ?? ""} ${staffToReset.lastname ?? ""}`.trim() || staffToReset.email);
        setPwDialogOpen(true);
      } else {
        console.error("[Reset Password] No password returned in response:", res.data);
      }
    } catch (e) {
...
// Put this helper near the top-level of the component file (above copyPwd)
async function copyToClipboardCrossBrowser(text: string): Promise<boolean> {
  ...
  // Legacy fallback (works on HTTP, older browsers)
  try {
...
// Replace your existing copyPwd with this:
const copyPwd = () => {
  // Instead of copying, prompt the user to select and copy manually
  setToastMsg("Please select and copy the password manually by long-pressing or right-clicking.");
        ...
        <Box>
                  ...
                  <TableRow key={s.id}>
                    ...
                    <TableCell align="right">
                      <Tooltip title="Reset Password">
                        <IconButton aria-label="reset-password" onClick={() => void resetPassword(s)}>
                          <LockResetIcon />
                        </IconButton>
                      </Tooltip>
          ...
          <Dialog open={staffDialogOpen} onClose={() => setStaffDialogOpen(false)} fullWidth maxWidth="md">
                ...
                <Grid item xs={12} sm={6}>
                  <FormControl fullWidth>
                    ...
                    <Select
                      labelId="role-select"
                      label="Select Role"
                      value={String(staffDraft.roleId)}
                ...
                <Grid item xs={12} sm={6}>
                  <FormControl fullWidth>
                    ...
                    <Select
                      ...
                      value={String(staffDraft.locationId)}
            ...
            <DialogActions sx={{ px: 3, pb: 3 }}>
              <Button onClick={() => setStaffDialogOpen(false)} sx={{ textTransform: "none" }}>
                Cancel
              </Button>
          ...
          <Dialog open={pwDialogOpen} onClose={() => setPwDialogOpen(false)} fullWidth maxWidth="xs">
            ...
            <DialogContent>
              ...
              <Typography variant="caption" sx={{ mt: 1, display: "block", color: "text.secondary" }}>
                Long-press or right-click to copy the password. It’s only shown once.
              </Typography>
            </DialogContent>
            <DialogActions>
              <Button onClick={() => setPwDialogOpen(false)}>Close</Button>
            </DialogActions>
          </Dialog>
          {/* Password reset confirmation dialog */}
          <Dialog open={confirmResetDialogOpen} onClose={() => setConfirmResetDialogOpen(false)} fullWidth maxWidth="xs">
            <DialogTitle>Confirm Password Reset</DialogTitle>
            <DialogContent>
              <Typography variant="body2">
                Are you sure you want to reset the password for{" "}
                {staffToReset
                  ? `${staffToReset.firstname ?? ""} ${staffToReset.lastname ?? ""}`.trim() || staffToReset.email
                  : "this user"}
                ?
              </Typography>
              <Typography variant="caption" sx={{ mt: 1, display: "block", color: "text.secondary" }}>
                This action will generate a new password, and the old password will no longer work.
              </Typography>
            </DialogContent>
            <DialogActions>
              <Button
              ...
              <Button variant="contained" onClick={confirmResetPassword} sx={{ minWidth: 96 }}>
                Confirm
              </Button>
            </DialogActions>
          </Dialog>
      ...
      {tab === 1 && (
        <Box>
          <Paper className="p-4 mb-4 flex justify-between gap-2 flex-wrap">
            ...
            <Button variant="contained" onClick={openCreateRole}>
              Add Role
            </Button>
          </Paper>
          <Paper className="p-4 overflow-x-auto">
      ...
      {tab === 2 && (
        <Box>
          <Paper className="p-4 mb-4 flex justify-between gap-2 flex-wrap">
            ...
            <Button variant="contained" onClick={openCreateLocation}>
              Add Location
            </Button>
          </Paper>
          <Paper className="p-4 overflow-x-auto">
